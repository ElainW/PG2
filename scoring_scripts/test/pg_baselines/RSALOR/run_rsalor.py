...
def main():
    ...
    REMOVE_PROPERTIES_IN_OUTPUT = ["DMS_score"]
    SEP = ","
    CHAIN = "A"
    MAX_CPUS = 16
    ...
    parser.add_argument("--DMS_index", type=int, default=-1, help="The index of DMS assay to process in the reference file")
    ...
    if args.DMS_index != -1:
        dataset_reference = dataset_reference.entries[args.DMS_index]
    ...
    if args.DMS_index == -1:
        ...
        # Init metadata
        dms_name = dataset_reference["DMS_id"]
        msa_name = dataset_reference["MSA_filename"]
        pdb_name = dataset_reference["pdb_file"]
        resid_shift = int(dataset_reference["MSA_start"]) - 1

        # Set paths
        dataset_input_path = os.path.join(args.DMS_data_folder, f"{dms_name}.csv")
        dataset_output_path = os.path.join(args.output_scores_folder, f"{dms_name}.csv")
        if os.path.exists(dataset_output_path):
            sys.exit(f"Already computed scores for {dms_name}")
        msa_path = os.path.join(args.MSA_folder, f"{msa_name}")
        pdb_path = os.path.join(args.DMS_structure_folder, f"{pdb_name}")
